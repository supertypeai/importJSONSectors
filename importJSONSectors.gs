// OPTION 1
// var apiKey = "YOUR_API_KEY" // Use your API Key here
 ...
 * @customfunction
 **/
function ImportCompanyIndex(index, query, parseOptions) {

  var header = {headers: {Authorization: apiKey}};
  var url = "https://api.sectors.app/v1/index/" + index;
...
function ImportCompanyReport(ticker, section, query, parseOptions) {
  ...
  // https://api.sectors.app/v1/company/report/bbca/?sections=financials
 ...
 * @customfunction
 **/
function ImportCompanyReportList(section, query, parseOptions) {
  var header = {headers: {Authorization: apiKey}};
...
function defaultTransform_(data, row, column, options) {
  // Assuming "symbol" is in column index 0 (adjust as needed)
  var symbolColumnIndex = 1; 

  if (column === symbolColumnIndex && data[row][column] == null) {
    // If it's the "symbol" column and the value is null, inherit from the previous row
    if (row > 1) { 
      data[row][column] = data[row-1][column]; // Duplicate the value from the previous row
    } else {
      data[row][column] = ""; // If it's the first row, set it as empty
    }
